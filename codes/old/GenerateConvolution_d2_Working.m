...
Delta_hat(1,1) = 1;
tmp=Delta_hat;
fprintf("Condition number of D: %.15f",cond(tmp));
...
DiagGate=qclab.qgates.MatrixGate(0:2*n ,tmp,"Diag Block Encoding");
...
invCirc=qclab.QCircuit(inverse_qft.nbQubits);
invCirc.push_back(inverse_qft);
inverse_qft.asBlock("InvQFT");
...
fprintf("Error between expected result and actual result: %.20f\n", error);
error
